...
export async function POST(req: Request, props: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Safeguard: User cannot mute themselves
    if (userId === adminUser.id) {
      return NextResponse.json({ error: "You cannot block comments on your own account" }, { status: 400 });
    }
    ...
    // Safeguard: Moderators cannot mute Admins or other Moderators
    if (adminUser.role === "moderator" && ["admin", "moderator"].includes(user.role)) {
      return NextResponse.json(
        { error: "Moderators cannot mute other moderators or administrators" },
        { status: 403 }
      );
    }
