...
export const SettingsForm = () => {
  ...
  const handleResetKarma = async () => {
    ...
    try {
      const res = await fetch("/api/profile", {
        ...
        body: JSON.stringify({ resetKarma: true }),
      });
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || "Failed to reset reputation karma");
      }
      if (data.user) {
        setDbUser(data.user);
      }
      setSuccessMessage("Your reputation karma has been reset to 0 in the database!");
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      setSuccessMessage("Account deleted successfully. Logging you out...");
      setTimeout(() => {
        handelSignOut();
      }, 1500);
    } catch (err: unknown) {
