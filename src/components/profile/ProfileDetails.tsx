...
export const ProfileDetails: React.FC<ProfileDetailsProps> = ({
  ...
  const handleShareProfile = async () => {
    if (!user) return;
    const profileUrl = `${window.location.origin}/profile?username=${user.username}`;
    
    if (navigator.share) {
      try {
        await navigator.share({
          title: `${user.name} (@${user.username}) on Chai Charcha`,
          text: `Check out ${user.name}'s profile on Chai Charcha!`,
          url: profileUrl,
        });
  ...
  return (
    <div className="w-full flex flex-col gap-4">
          ...
          <div className="space-y-4">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                {user.role && (
                  <span className={`inline-flex items-center rounded-md border px-2 py-0.5 text-[8px] font-black uppercase tracking-wider ${roleColors[user.role]}`}>
                    {user.role}
                  </span>
                )}
              </div>
      ...
      {/* Profile Actions Card Container */}
      <div className="w-full p-4 rounded-3xl border border-(--card-border) bg-(--card-background)/40 backdrop-blur-xs shadow-md transition-all duration-300 hover:shadow-lg">
        <div className="flex flex-wrap gap-2.5 w-full">
          {isOwnProfile ? (
            <>
              <Link
                href="/settings"
                className="flex-1 min-w-[120px] flex items-center justify-center gap-2 px-4 py-2.5 bg-linear-to-r from-orange/10 to-spicy-paprika/10 hover:from-orange/20 hover:to-spicy-paprika/20 border border-orange/20 hover:border-orange/40 rounded-xl text-sm font-bold text-orange hover:text-orange-600 transition-all duration-200 cursor-pointer active:scale-95 whitespace-nowrap"
              >
              ...
              <button
                ...
                className="flex-1 min-w-[100px] flex items-center justify-center gap-2 px-4 py-2.5 bg-linear-to-r from-orange/10 to-spicy-paprika/10 hover:from-orange/20 hover:to-spicy-paprika/20 border border-orange/20 hover:border-orange/40 rounded-xl text-sm font-bold text-orange hover:text-orange-600 transition-all duration-200 cursor-pointer active:scale-95 whitespace-nowrap"
              >
                <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 8.25H7.5a2.25 2.25 0 0 0-2.25 2.25v9a2.25 2.25 0 0 0 2.25 2.25h9a2.25 2.25 0 0 0 2.25-2.25V10.5A2.25 2.25 0 0 0 16.5 8.25H15M12 3v11.25m0-11.25L9.75 5.25M12 3l2.25 2.25" />
                </svg>
                <span>Share</span>
              </button>
            </>
            ...
            <>
              <button
                ...
                className={`flex-1 min-w-[100px] flex items-center justify-center gap-1.5 px-4 py-2.5 border rounded-xl text-xs font-bold transition-all duration-200 cursor-pointer active:scale-95 ${
                  isFollowing
